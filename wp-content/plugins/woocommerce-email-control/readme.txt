...
= 3.33 =
* Fix issue where Shipment Tracking would not display details when using the Deluxe theme.

= 3.32 =
* Fix issue when using the hook 'plugins_loaded'.

= 3.31 =
* Fix bug on some installations "Using $this when not in object context".
...
* Update to `get_coupon_codes()` method.
* Email Customizer now requires WooCommerce version 3.0.8 or above.
* Remove the old `below-wc2.5` templates - we don't need these for backwards compatibility any longer.
