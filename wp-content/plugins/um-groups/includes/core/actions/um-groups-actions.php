...
function um_groups_join_button( $group_id = 0 ) {
	...
	if ( um_groups_admin_all_access() || um_user( 'ID' ) != get_current_user_id() ) {
		return;
	}
	...
	switch ( $privacy ) {
		...
		case 'private':
			if ( is_user_logged_in() ) {
				...
				<a href="<?php echo add_query_arg( array( 'redirect_to', get_the_permalink( $group_id ) ), um_get_core_page( 'login' ) ) ?>"
