...
function um_multiselect_options_user_tags( $options, $data ) {
	...
	/**
	 * Restore new user tags after form validation fails. 2020-04-11
	 */
	if ( isset( UM()->form()->errors ) && isset( UM()->form()->post_form['submitted'] ) ) {
		$key = $data['metakey'];
		if ( ! empty( UM()->form()->post_form['submitted'][ $key ] ) ) {
			$submitted = UM()->form()->post_form['submitted'][ $key ];
			$new_utags = array_diff( $submitted, array_keys( $options ) );
			$options = array_merge( $options, array_combine( $new_utags, $new_utags ) );
		}
	}
