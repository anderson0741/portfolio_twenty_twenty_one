...
class WPGMapsImportExport{
	...
	public function export_markers(){
        ...
        foreach ( $results as $data ) {
			...
			if(!empty($data['id'])){
            	$formattedCats = "";
            	$categoryTable = $wpdb->prefix . "wpgmza_markers_has_categories";
            	$categories = $wpdb->get_results("SELECT * FROM `{$categoryTable}` WHERE `marker_id`=" . intval($data['id']));

            	if(!empty($categories)){
            		foreach ($categories as $cdata) {
            			if(!empty($cdata->category_id)){
            				if(!empty($formattedCats)){
            					$formattedCats .= ",";
            				}
            				$formattedCats .= $cdata->category_id;
            			}
            		}
            	}
